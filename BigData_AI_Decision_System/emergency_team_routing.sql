        ...
        LEAST(
            ...
            GREATEST(
                ...
                (
                    CASE
                        WHEN avg_temp > 40
                        OR fc_temp_next6h > 40 THEN 15
                        WHEN avg_temp > 35
                        OR fc_temp_next6h > 35 THEN 10
                        ELSE 0
                    END
                ) + (
                    CASE
                        WHEN avg_precip > 100
                        OR fc_precip_next6h > 100 THEN 15
                        WHEN avg_precip > 80
                        OR fc_precip_next6h > 80 THEN 10
                        ELSE 0
                    END
                ) + -- Pressure indicators (5% weight)
                (
                    CASE
                        WHEN avg_pressure < 990 THEN 5
                        WHEN avg_pressure < 1000 THEN 3
                        ELSE 0
                    END
                ) + -- Satellite imagery confirmation (5% weight)
                (
...
team_allocation_analysis AS (
    SELECT *,
        ...
        END AS required_team_composition
    FROM imminent_hazard_analysis
    WHERE imminent_danger_score >= 50
),
geographic_response_routing AS (
    SELECT *
    FROM team_allocation_analysis
